		...
		const char *buf, size_t count)
{
	...
	bma250_set_selftest_stn(bma250->bma250_client,0); /* positive direction*/
	mdelay(10);
	bma250_read_accel_x(bma250->bma250_client,&value1);
	bma250_set_selftest_stn(bma250->bma250_client,1); /* negative direction*/
	mdelay(10);
...
static void bma250_irq_work_func(struct work_struct *work)
{
	...
	case 0x01: printk("Low G interrupt happened\n");
	...
	case 0x02: for(i=0;i<3;i++)
		   {
			   ...
			   printk("High G interrupt happened,exis is %d,first is %d,sign is %d\n",i,first_value,sign_value);
		   }
		   break;
	case 0x04: for(i=0;i<3;i++)
		   {
			   ...
			   printk("Slop interrupt happened,exis is %d,first is %d,sign is %d\n",i,first_value,sign_value);
		   }
		   break;

	case 0x10: 	printk("double tap interrupt happened\n");
			input_report_rel(bma250->input,DOUBLE_TAP_INTERRUPT,DOUBLE_TAP_INTERRUPT_HAPPENED);
			break;
	case 0x20: 	printk("single tap interrupt happened\n");
	...
	case 0x40:  bma250_get_orient_status(bma250->bma250_client,&first_value);
		   printk("orient interrupt happened,%s\n",orient[first_value]);
		...
		const struct i2c_device_id *id)
{
	...
	if (tempvalue != 0xFF) {
		printk(KERN_INFO "Bosch Sensortec Device(ID=%d) detected!\n"
